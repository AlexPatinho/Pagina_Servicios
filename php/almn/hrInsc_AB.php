...
$semestre = '20231';//SEMESTRE DE PAGO
	...
	if(!$conex){
		...
		if($art22 == "t"){
			...
			if(intval($noInsc) == -1){//NO SE HA REGISTRADO EL PAGO DE CUOTA ANUAL
				...
				s.carr AS \"NCRR\",".
				" c.nombre AS \"CRR\",".
				" CASE WHEN dr.sis = 'SUA' THEN '132.247.154.41'".
				...
				" FROM sorteo_ab s, carrera c, diralum dr ".
...
/*----------------------------------------------------------------*/
	...
	$r = false;
	for($i = 0; $i < 5; $i++){
		if($_POST["nCrr"] == $_SESSION["hrInsc"][$i]["NCRR"] &&  $_POST["hrInsc"] == $_SESSION["hrInsc"][$i]["FCHINSC"])
		{//SE VERIFICA QUE LA(S) HORA(S) DE INSCRIPCION DEL ALUMNO SEAN VALIDAS
			$r = true;
			break;
		}
	}
