...
//DECLARAMOS EL PERIODO: 1 PARA REINSCRIPCION, 2 PARA ALTAS Y BAJAS, 6 PARA REMEDIALES
$periodo = '2';
	...
	and isset($_POST["tipo"])){
		...
		*/
		...
		if(!$conex){
				...
				case "HIN": {
					...
					*/
					...
					$aux1 = "hora_final = substring((CURRENT_TIME + time '01:00:00')::text FROM 1 FOR 8)::time, 
							realizado = FALSE, reabierto= FALSE, dia_inscripcion = current_date, hora_inicio='00:01:00' ";
			...
			if(true){//SE ENCUENTRA EN EL DIA DE LA INSCRIPCION
				if(true){//LA HORA ACTUAL ES MAYOR O IGUAL A LA DE INSCRIPCION
					/*
					...
					*/
					...
					if ($pln != '1325') //colocar 1325 el segundo dia de altas y bajas 
					{
						$sentencia .= "AND tipo_inscripcion_id = '$periodo'";
						json_encode(console.log($sentencia));
					}
					...
					if(pg_affected_rows($ex) > 0){
						$sql_2 ="UPDATE alumno SET \"password\" = md5('123456') where id ='$cta' ";
						$resp  = pg_query($conex,$sql_2) or die(json_encode("FALLO AL ASIGNAR CONTRASEÑA: " . pg_last_error()));

						$val = "D";//LA ACTUALIZACION DE HORARIO SE REALIZO CORRECTAMENTE
					} else {
						$val = "$resp";//YA SE HA HECHO UNA REAPERTURA ANTERIOR
					}
				} else {
