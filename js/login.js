...
$(function(){
		...
		//CONTROLAMOS LAS OPCIONEES DE LA ANIMACION DE LOS MENSAJES INFORMATIVOS
		var $efecto = 'drop';
		...
		$('#login').submit(function(e) {
			...
			if (_us == ''){
				$('#anncs').html('<div id="msj" class="warning">INGRESE UN N&Uacute;MERO DE CUENTA</div>');
				$('#msj').hide().show($efecto, $opciones);
				...
				setTimeout(function(){
					$.ajax({
						...
						success : function(results) {
							...
							$('#msj').hide().show($efecto, $opciones);	
							if(results == '-2'){
								$('#anncs').html('<div id="msj" class="error" >N&Uacute;MERO DE CUENTA INCORRECTO</div>');
								$('#msj').hide().show($efecto, $opciones);	
								$('#pass').val('');
						...
						error: function(xhr, ajaxOptions, thrownError){	
							if(xhr.responseText === 'NULL'){
								...
								$('#anncs').html('<div id="msj" class="warning" >EL USUARIO NO SE HA REGISTRADO EN EL SISTEMA.<br />FAVOR DE <a href="./registro.php">REGISTRARSE.</a></div>');
								...
								$('#anncs').empty();
								//$('#anncs').html('<div id="msj" class="error" >' + ajaxOptions + ': ' + xhr.status + ' - ' + thrownError + '</div>');
								$('#anncs').html('<div id="msj" class="error" >DATOS INGRESADOS NO VALIDOS.<br />FAVOR DE CONTACTAR CON SERVICIOS ESCOLARES.</div>');
