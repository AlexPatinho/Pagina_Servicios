			...
			if (_us == ''){
				...
				setTimeout(function(){
					$.ajax({
						...
						dataType : 'json',
						data : {usr: _us, pass: _ps},
						success : function(results) {
							console.log(results);
							
							$('#anncs').html('<div id="msj" class="success" >OK</div>');
							$('#msj').hide().show($efecto, $opciones);
							...
							if(results == 'A'){
								...
								$('#anncs').html('<div id="msj" class="warning" >NO SE PUEDE INGRESAR AL SISTEMA DESDE &Eacute;STA DIRECCI&Oacute;N IP. </div>');
								$('#msj').hide().show($efecto, $opciones);
								$('#usr').val('');
								$('#pass').val('');
								$('#usr').focus();
