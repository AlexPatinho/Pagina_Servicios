    ...
    $.fn.formToWizard = function( options, cmdParam1 ) {
        ...
        if( typeof options !== 'string' ) {
            ...
            steps.each(function(i) {
                ...
                if( options.showProgress ) {
                    if( options.showStepNo )
                        $("#steps").append("<li id='stepDesc" + i + "'>" + options.stepTitleName + " " + (i + 1) + "<span>" + $(this).find("legend").html() + "</span></li>");
                    else
        ...
        function createPrevButton(i) {
            var stepName = "step" + i;
            $("#" + stepName + "commands").append("<a href='#' id='" + stepName + "Prev' class='prev " + options.buttonClass + "'>" + options.prevBtnName + "</a>");

            $("#" + stepName + "Prev").bind("click", function(e) {
				//----------------------------------
                $("#" + stepName).hide('drop', function(){
						$("#step" + (i - 1)).show('drop', {direction: 'right'});
				});
        ...
        function createNextButton(i) {
            var stepName = "step" + i;
            $("#" + stepName + "commands").append("<a href='#' id='" + stepName + "Next' class='next " + options.buttonClass + "'>" + options.nextBtnName + "</a>");

            $("#" + stepName + "Next").bind( "click", function(e) {
                if( options.validateBeforeNext() === true ) {
					//--------------------------------------------
                    $("#" + stepName).hide('drop', {direction: 'right'}, function(){
						 $("#step" + (i + 1)).show('drop');
					});
                   //---------------------------------------------
                    if (i + 2 == count)
                        $(submmitButtonName).show('drop');
                    selectStep(i + 1);
                }
