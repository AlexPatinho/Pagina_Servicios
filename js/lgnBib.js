				...
				setTimeout(function(){
					$.ajax({
						...
						success : function(results) {
							...
							if(results == 'A'){
							...
							} else if (results == 'C' || results == 'D'){
								//EL USUARIO NO POSEE LOS PERMISOS PARA INGRESAR AL SISTEMA
								$('#anncs').html('<div id="msj" class="warning" >NO POSEE LOS PERMISOS NECESARIOS PARA ACCEDER AL SISTEMA</div>');
								$('.unbl').removeAttr('disabled');
								$('#msj').hide().show($efecto, $opciones);
								$('#usr').val('');
								$('#pass').val('');
								...
								//LOS PERMISOS Y EL USUARIO SON CORRECTOS
								$('#anncs').html('<div id="msj" class="succes" >OK</div>');
								$('#msj').hide().show($efecto, $opciones);	
								window.location.replace('usr.php');
							} else {
								//SE GENERO ALGUN ERROR EN LA CONEXION CON EL SERVIDOR
